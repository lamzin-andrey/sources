...
function _mysqlParseSelect(s) {
	...
	if (defined('MYSQL_EMU_SOURCE_FILE')) {
		...
		/*var nn = 0;
		mysql.ival = setInterval(function(){
			if (window.rawdbinfo && parseInt(window.rawdbinfo.size)) {
				_mysqlOnParseSelect();
				clearInterval(mysql.ival);
			} else {
				mysql.progress_callback('Tick ' + nn, -1);
				nn++;;
			}
		}, 1000);*/
	}
 ...
 * 
*/
function onLoadPartTable() {//alert('onLoadPartTable');
...
function _mysqlIncludePart() {
	...
	/*window.rawdbdata = false;
	mysql.ival2 = setInterval(function(){
		if(window.rawdbdata) {
			clearInterval(mysql.ival2);
			onLoadPartTable();
		} else {
			mysql.progress_callback('Tick ' + nn, '-2');
			nn++;
		}
	}, 1000);*/
