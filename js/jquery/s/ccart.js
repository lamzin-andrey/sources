 ...
 */
	setPhoneInputMask: function setPhoneInputMask() {
		var arr = AppACME.Cart.getPhonesInputs(),
		    i,
		    a,
		    ms = '+7',
		    curr = $('#currency').val(),
		...
		for (i = 0; i < arr.length; i++) {
			...
			a = {
				onKeyPress: function onKeyPress(b, c, d) {
 ...
 */
	setMinimalOrderMessage: function setMinimalOrderMessage() {
		var cart = AppACME.Cart,
		    s,
		    total,
		    n,
		    rawTotal;
  ...
  *  Вызывается в Cart.setOrderButton в том случае, если форма оформления заказа не содержит варианта оплаты картами, так как при его использовании это сообщение выводится в результате валидации черновика
 */
	setShopIsNotWorkNowMessage: function setShopIsNotWorkNowMessage() {
		var cart = AppACME.Cart,
		    s,
		    k = 'ShopIsNotWorkNowMessageTS',
		    n,
 ...
 */
	showError: function showError(error, messages) {
		var s = error,
		    aMessages = [],
		    i,
		    o = AppACME.Cart,
		    sCurr = $(o.HTML_SHOP_CURRENCY_INP_ID).val(),
		    sCurrSign;
 ...
 */
	showAlertRequireLocation: function showAlertRequireLocation() {
 ...
 */
	setOrderButton: function setOrderButton() {
		var o = AppACME.Cart,
		    cc = AppACME.CoreCart;
 ...
 */
	runUnworkMessageTimer: function runUnworkMessageTimer() {
 ...
 */
	onTickData: function onTickData(d) {
 ...
 */
	onTickReqFail: function onTickReqFail() {
 ...
 */
	onRestoreOrderFormData: function onRestoreOrderFormData() {
		var f = AppACME,
		    o = f.Cart,
		    cc = f.CoreCart,
		    locForm = f.geo.maingeoform,
 ...
 */
	setOrderFormField: function setOrderFormField() {
		var o = AppACME.Cart,
		    a = $('#ofi7'),
		    b = $('#ofi8'),
		    c = $('#ofi9'),
 ...
 */
	requestCartItems: function requestCartItems() {
		var f = AppACME,
		    cc = f.CoreCart,
		    o = f.Cart,
		    s = '',
		...
		if (cart.length || ballsCart.length) {
			s = cart.join(',');
			q = ballsCart.join(',');
			Lib._get(cc.onRequestCartItemSuccess, '/checkitemsjson?m=' + s + '&b=' + q, function () {});
		}
	},
 ...
 */
	onRequestCartItemSuccess: function onRequestCartItemSuccess(data) {
		if (data[2] != 'done') {
			return;
		}
		var f = AppACME,
		    cc = f.CoreCart,
		    cartItems = array_fill_keys(data[0], 1),
		    ballsItems = array_fill_keys(data[1], 1),
		    balls = cc.getStoredCartBallsObject(),
		    cart = cc.getStoredCartObject(),
		    i = void 0,
		    newCart = {},
		    newBalls = {},
 ...
 */
	getStoredCartObject: function getStoredCartObject() {
		var o = AppACME.Cart,
		    cart = o.Lib.storage(o.getShopId());
		cart = cart ? cart : {};
		return cart;
	},
 ...
 */
	getStoredCartBallsObject: function getStoredCartBallsObject() {
		var o = AppACME.Cart,
		    s = o.getShopId().replace('cart-', ''),
		    cart = o.Lib.storage(o.STORAGE_BALL_CART_PREFIX + s);
		cart = cart ? cart : {};
		return cart;
	},
 ...
 */
	saveCart: function saveCart(сart) {
		var o = AppACME.Cart;
  ...
  * @description Сохраняет объект с данными о товарах за баллы отложенных в корзину
 */
	saveBallsCart: function saveBallsCart(сart) {
		var o = AppACME.Cart,
		    s = o.STORAGE_BALL_CART_PREFIX + String(o.getShopId()).replace('cart-', '');
 ...
 */
	runMinutesTimer: function runMinutesTimer() {
		var cc = AppACME.CoreCart,
		    i = void 0,
		    f = void 0;
		cc._minInterval = setInterval(function () {
		...
		cc._min10Interval = setInterval(function () {
 ...
 */
	setQuantityEachProductInCart: function setQuantityEachProductInCart() {
		var o = AppACME.Cart,
		    Lib = o.Lib,
		    list = $(o.HTML_CART_ITEM_QUANTITY_CSS),
		    vPrices = $(o.HTML_CART_ITEM_PRICE_CSS),
		    cart,
		    total = 0,
		    n,
		    currency,
		    priceStorageKey = 'price_' + o.getShopId(),
		    price,
		    storageKey,
		    cid,
		    totalBalls = 0,
			...
			if (cart instanceof Object) {
				...
				if (vPrices[0]) {
					...
					if (totalBalls) {
                        ...
                        $(o.HTML_CART_TOTAL_BALL_CURRENCY_ID).addClass(o.HTML_GOODS_COIN_ICON_CSS.replace('.', ''));
                        $(o.HTML_CART_TOTAL_BALL_CURRENCY_ID).html('');
					} else {
						$(o.HTML_CART_TOTAL_BALL_WRAPPER_ID).addClass('hide');
					}
				}
