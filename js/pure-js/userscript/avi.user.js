...
w.onload = function() {
  ...
  transportType() + 
  i('Заголовок', 'iTitle') +
  '<textarea id="iBody" name="iBody" style="resize:none;width:650px;height:250px" rows="10"></textarea>' +
  i('Цена', 'iPrice') +
  '<input type="file" id="iPhoto" name="iPhoto">' +
  i('Телефон', 'iPhone') +
  '</form></div>';
...
function createExportForm() {
  ...
  e('iBody').value = d.getElementsByClassName('item-description-text')[0].innerHTML;
  normalizeIBody();
  e('iPrice').value = d.getElementsByClassName('js-item-price')[0].innerText;
  normalizeIPrice();
}
function normalizeIPrice() {
  var s = e('iPrice').value, i = parseInt(s), inTag, q = '', ch;
  if (isNaN(i)) {
    i = 1;
  }
  e('iPrice').value = i;
