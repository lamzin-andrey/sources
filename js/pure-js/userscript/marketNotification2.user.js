// ==UserScript==
// @name        Burime
// @namespace   http://erakiri.lan
// @include     http://erakiri.lan
// @version     1
// @grant       none
// ==/UserScript==
...
window.onload = function() {
  // alert('It work');
  // CONFIG
  var lowLevelPrice = 48000;
  var topLevelPrice = 49000;
  var space = 100; //например если lowLevelPrice = 150 а space = 2 уведомления будут показыватсья в коридоре от 150 до 152
  var secondsBetweenAlerts = 30;//30;
  ...
  setInterval(() =>{
    ...
    for (j = 0; j < lswrapp.length; j++) {
        ...
        if (div.hasAttribute('class')
					&& div.getAttribute('class').indexOf('totalindex___') != -1 
					&& (
          			div.getAttribute('class').indexOf('redindex_') != -1 
          			|| div.getAttribute('class').indexOf('greenindex_') != -1 
             )
				)
		{
			var fPrice = parseFloat( strip_tags( String(div.innerHTML).trim()  ) );
      // alert('`' + String(div.innerHTML).trim() + '`');
      // alert(fPrice);
			if ( (fPrice >= lowLevelPrice && fPrice <= (lowLevelPrice + space)) ||
				 ( fPrice <= topLevelPrice && fPrice >= (topLevelPrice - space) )
			) 
      		{
				if (!window.lastTs || (time() - window.lastTs > secondsBetweenAlerts) ) {
					window.SimpleNotice.show('BT price = ' + fPrice + '!', 'Hop!', laavatar);
				  	window.lastTs = time();
				}
			}// end if ( (fPrice >= lowLevelPrice && fPrice <= (lowLevelPrice + space)) || ...
            
            break;
        } // END if (div.hasAttribute('class')..
    } // endfor
