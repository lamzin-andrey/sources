...
class LocalhostManager(Gtk.Window):
    ...
    def showMkCertHelp(self):
        shell = "!#/bin/bash\n"
        shell += "xdg-open " + App.dir() + f"/help/mkcert/{self.locale}.html"+ "\n"
        shellFile = App.dir() + "/p/u.sh"
        self.write_file(shellFile, shell)
    ...
    def openCaHelp(self):
        shell = "!#/bin/bash\n"
        shell += "xdg-open " + App.dir() + f"/help/rootCert/{self.locale}.html"+ "\n"
        shellFile = App.dir() + "/p/u.sh"
        self.write_file(shellFile, shell)
        Env.exec(shellFile, self.onCertStdOut, self.onCertStdOut, self.onCertStdOut);
    ...
    def parsePhpIni(self):
        ...
        if aLinesCaCert.length == 2 and not self.rootCANotFound and self.originalCertFound:
            return True
        return False
