...
class LocalhostManager(Gtk.Window):
    ...
    def load_localization(self):
        ...
        # Запоминаем локаль
        s = trim(self.read_file(App.dir() + "/locale"));
        if s != "ru" and s != "en":
            s = "en"
        self.locale = s;
    ...
    def on_add_clicked(self, button):
        ...
        # Пробуем запустить mkcert
        shell = "#!/bin/bash\n"
        shell += "cd /tmp\n"
        shell += f"mkcert {site_name}\n"
        #print("App.dir: " + App.dir())
        shellFile = App.dir() + "/p/u.sh"
        self.write_file(shellFile, shell)
        self.siteName = site_name
        self.rootDir = root_dir
        Env.exec(shellFile, self.onCertCreated, self.onCertStdOut, self.onCertStdErr);
        #self.addNoSslSite(site_name, root_dir)
    
    def onCertCreated(self, stdout, stderr2):
        print("onCertCreated called " + self.rootDir);
        #self.show_info("onCertCreated", stdout + "\n\n===\n\n" + stderr2);
        
        if file_exists(f"/tmp/{self.siteName}.pem") and file_exists(f"/tmp/{self.siteName}-key.pem"):
            self.addSslSite(self.siteName, self.rootDir)
            #self.show_info("Info", "Will Add SSL");
        else:
            if self.show_question("Question", self.t("mkcert_help_offer")):
                self.showMkCertHelp();
                #self.show_info("Info", "Will showMkDirHelp");
            else:
                self.addNoSslSite(self.siteName, self.rootDir)
        
    def onCertStdOut(self, stdout):
        a = 1
        
    def onCertStdErr(self, stderr):
        a = 1
        
    def showMkCertHelp(self):
        shell = "!#/bin/bash\n"
        helpFile = App.dir() + f"/help/mkcert/{self.locale}.html";
        if (file_exists(App.dir() + "/../../d/docDir.txt")):
            s = trim(FS.readfile(App.dir() + "/../../d/docDir.txt"))
            if (file_exists(s) and os.path.isdir(s)):
                helpFile = f"{s}/xamppSsl/mkcert/{self.locale}.html";
        shell += "xdg-open " + helpFile + "\n"
        shellFile = App.dir() + "/p/u.sh"
        self.write_file(shellFile, shell)
        Env.exec(shellFile, self.onCertStdOut, self.onCertStdOut, self.onCertStdOut);
    
    def addSslSite(self, site_name, root_dir):
        # Добавляем сайт
        self.names.append(site_name)
        self.all_sites.append(site_name)
        
        # Создаем директорию
        self.create_directory(site_name, root_dir, True)
        ...
        # Обновить сертификат для php скриптов
        self.updateRootCertificate()
        #if False == self.updateRootCertificate():
            # Показываем сообщение об успехе, если обновление сертификата пройдет успешно, это также произойдёт в другом месте
            #self.show_info("Success", f"Site '{site_name}' added successfully")
    
    def updateRootCertificate(self):
        self.originalCertFound = True
        self.rootCANotFound = False
        self.combinedCert = "/etc/ssl/certs/combined-ca-bundle.crt"
        self.createDir("/etc/ssl/certs");
        user = self.getUser();
        self.stdRootCert = f"/home/{user}/.local/share/mkcert/rootCA.pem"
        if self.parsePhpIni():
            print("parseOhoIni success")
            rShell = "#!/bin/bash\n"
            rShell += f"cp {self.originalPath} {self.combinedCert}\n"
            rShell += f"cat {self.stdRootCert} | sudo tee -a {self.combinedCert}\n"
            shellFile = App.dir() + "/p/rt.sh";
            self.write_file(shellFile, rShell);
            
            shell = "#!/bin/bash\n"
            shell += f"pkexec {shellFile}\n"
            shellFile = App.dir() + "/p/u.sh";
            self.write_file(shellFile, shell);
            Env.exec(shellFile, self.onRootCertUpdate, self.onCertStdOut, self.onCertStdErr)
            return True
        else:
            print("parseOhoIni fail")
            msg = self.t("php_ini_unexpected_content", "php_ini_unexpected_content") + "\n"
            if self.originalCertFound == False:
                sFile = self.t("File", "File");
                sNF = self.t("not_found", "not found");
                msg += f"(1) {sFile} {self.originalCertFile} {sNF}!\n"
            if self.rootCANotFound:
                sNF = self.t("not_found_std_file", "Not found standart file");
                msg += f"(2) {sNF} /home/{user}/.local/share/mkcert/rootCA.pem\n"
            yes = self.t("Yes", "Yes")
            msg += self.t("Press", "Click") + f" \"{yes}\" " + self.t("php_ini_unexpected_content_2", "to view the help and update the root certificate manually. The help also explains how to edit php.ini to update the root certificate automatically.") + "\n";
            if self.show_question("Question", msg):
                self.openCaHelp();
            return False

    def onRootCertUpdate(self, stdout, stderr):
        if "" == trim(stderr):
            sSite = self.t("Site", "Site")
            addSucc = self.t("added_successfully", "added successfully")
            if self.aLinesCaCert.length == 1:
                s = "#original\n"
                if self.hasTagOriginal:
                    s = ""
                s += "#openssl.cafile=" + self.originalPath + "\n"
                s += "openssl.cafile=" + self.combinedCert
                self.aPhpIni[self.aLinesCaCert[0]] = s
            elif self.aLinesCaCert.length == 2:
                if not self.hasTagOriginal:
                    s = "#original\n"
                    s += "#openssl.cafile=" + self.originalPath
                else:
                    s = "#openssl.cafile=" + self.originalPath
                self.aPhpIni[self.aLinesCaCert[0]] = s
                s = "openssl.cafile=" + self.combinedCert + "\n"
                self.aPhpIni[self.aLinesCaCert[1]] = s
            s = implode("\n", self.aPhpIni)
            self.write_file("/opt/lampp/etc/php.ini", s);
            #self.show_info(self.t("Success", ""), f"{sSite} '{self.siteName}' {addSucc}")
        else:
            msg = self.t("root_cert_shell_error", "The root certificate was not updated because an error occurred while executing the script.") + "\n"
            msg += trim(stderr) + "\n"
            yes = self.t("Yes", "Yes")
            msg += self.t("Press", "Click") + f" \"{yes}\" " + self.t("php_ini_unexpected_content_2", "to view the help and update the root certificate manually. The help also explains how to edit php.ini to update the root certificate automatically.") + "\n";
            if self.show_question(self.t("Question", "Question"), msg):
                self.openCaHelp()
    
    def openCaHelp(self):
        shell = "!#/bin/bash\n"
        helpFile = App.dir() + f"/help/rootCert/{self.locale}.html"
        if (file_exists(App.dir() + "/../../d/docDir.txt")):
            s = trim(FS.readfile(App.dir() + "/../../d/docDir.txt"))
            if (file_exists(s) and os.path.isdir(s)):
                helpFile = f"{s}/xamppSsl/rootCert/{self.locale}.html";
        shell += "xdg-open " + helpFile + "\n"
        shellFile = App.dir() + "/p/u.sh"
        self.write_file(shellFile, shell)
        Env.exec(shellFile, self.onCertStdOut, self.onCertStdOut, self.onCertStdOut);
    ...
    def parsePhpIni(self):
        ini = "/opt/lampp/etc/php.ini"
        self.ini = ini
        if False == file_exists(ini):
            self.show_error("Error", f"Not found {ini}")
            return False
        c = self.read_file(ini)
        lines = explode("\n", c)
        
        prevLine = ''
        originalPath = ''
        originalPathFromComment = False
        self.hasTagOriginal = False
        aLinesCaCert = Array();
        i = 0;
        for line in lines:
            if strpos(line, 'openssl.cafile=') != -1:
                aLinesCaCert.push(i);
                if originalPath == '':
                    originalPath = self.getIniValue(line)
                    if trim(prevLine) == '#original':
                        self.hasTagOriginal = True
            prevLine = line
            i = i + 1;
        if aLinesCaCert.length > 2 or aLinesCaCert.length == 0:
            return False
        self.aPhpIni = lines
        self.originalCertFound = file_exists(originalPath)
        self.originalPath = originalPath
        self.rootCANotFound = not file_exists(self.stdRootCert)
        self.aLinesCaCert = aLinesCaCert
        if (aLinesCaCert.length == 2 or aLinesCaCert.length == 1) and not self.rootCANotFound and self.originalCertFound:
            return True
        return False
    ...
    def addNoSslSite(self, site_name, root_dir):
        # Добавляем сайт
        self.names.append(site_name)
        self.all_sites.append(site_name)
    ...
    def create_directory(self, site_name, root_dir, useSsl = False):
        ...
        try:
            ...
            if useSsl:
                ssl_path = f"{base_path}/{root_dir}/ssl"
...
\tSSLEngine on
\tSSLCertificateFile "/opt/lampp/htdocs/{HOST}/{www}/ssl/{HOST}.pem"
\tSSLCertificateKeyFile "/opt/lampp/htdocs/{HOST}/{www}/ssl/{HOST}-key.pem"
</VirtualHost>
...
"""
        ...
        for site_name in self.names:
            ...
            if root_dir == '':
                ...
                if False == file_exists(f"/opt/lampp/htdocs/{site_name}/{root_dir}"):
                    root_dir = self.rootDir
            print(f"\n\n932Line:root_dir from cfg get as {root_dir}\n\n")
            
            # Добавляем в vhosts
            site_config = template.replace("{HOST}", site_name).replace("{www}", root_dir)
            vhosts_content += site_config
            
            # Если существует сертификат в каталоге ssl, добавляем конфигурацию хоста
            if False == file_exists(f"/opt/lampp/htdocs/{site_name}/{root_dir}/ssl/{site_name}.pem"):
                # но если не существует, поищем его в /tmp
                if file_exists(f"/tmp/{site_name}.pem") and file_exists(f"/tmp/{site_name}-key.pem"):
                    shutil.copy2(f"/tmp/{site_name}.pem", f"/opt/lampp/htdocs/{site_name}/{root_dir}/ssl/{site_name}.pem")
                    shutil.copy2(f"/tmp/{site_name}-key.pem", f"/opt/lampp/htdocs/{site_name}/{root_dir}/ssl/{site_name}-key.pem")
                    
            if file_exists(f"/opt/lampp/htdocs/{site_name}/{root_dir}/ssl/{site_name}.pem"):
                site_config = templateSsl.replace("{HOST}", site_name).replace("{www}", root_dir)
                vhosts_content += site_config
