module FileUtils
  ...
  METHODS = ["cp_r", "move", "remove_dir", "symlink", "ln_s", "rmdir", "cmp", "rm_r", "remove_entry_secure", "copy_file", "chmod_R", "makedirs", "copy_entry", "rm_f", "chown", "getwd", "uptodate?", "compare_file", "mkdir_p", "rm", "ln_sf", "rm_rf", "cd", "copy", "remove_entry", "touch", "link", "remove_file", "mkdir", "copy_stream", "ln", "safe_unlink", "compare_stream", "install", "pwd", "remove", "mkpath", "identical?", "cp", "mv", "chown_R", "chmod", "chdir", "rmtree"]
  NoWrite = FileUtils::NoWrite
  OPT_TABLE = {"rm_rf"=>[:noop, :verbose, :secure], "mv"=>[:force, :noop, :verbose, :secure], "cp"=>[:preserve, :noop, :verbose], "ln_s"=>[:force, :noop, :verbose], "cd"=>[:verbose], "chown"=>[:noop, :verbose], "makedirs"=>[:mode, :noop, :verbose], "mkdir_p"=>[:mode, :noop, :verbose], "chown_R"=>[:noop, :verbose, :force], "link"=>[:force, :noop, :verbose], "rm"=>[:force, :noop, :verbose], "cp_r"=>[:preserve, :noop, :verbose, :dereference_root, :remove_destination], "mkdir"=>[:mode, :noop, :verbose], "touch"=>[:noop, :verbose, :mtime, :nocreate], "chmod_R"=>[:noop, :verbose, :force], "rmdir"=>[:noop, :verbose], "chmod"=>[:noop, :verbose], "install"=>[:mode, :preserve, :noop, :verbose], "ln_sf"=>[:noop, :verbose], "move"=>[:force, :noop, :verbose, :secure], "copy"=>[:preserve, :noop, :verbose], "symlink"=>[:force, :noop, :verbose], "safe_unlink"=>[:noop, :verbose], "ln"=>[:force, :noop, :verbose], "mkpath"=>[:mode, :noop, :verbose], "chdir"=>[:verbose], "rmtree"=>[:noop, :verbose, :secure], "rm_r"=>[:force, :noop, :verbose, :secure], "rm_f"=>[:noop, :verbose], "remove"=>[:force, :noop, :verbose]}
